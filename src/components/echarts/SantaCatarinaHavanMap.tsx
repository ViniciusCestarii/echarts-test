...
const mapOption = {
  ...
  title: {
    text: 'Havan em Santa catarina usando Havan API',
    left: '1%'
  },
  geo: {
...
const SantaCatarinaHavanMap = () => {
  const chartRef = useRef(null);
  const echartRef =  useRef<echarts.ECharts | null>(null);

  useEffect(() => {
    const fetchScFiliais = async () => {
      const response = await fetch('https://apigatewaywebapi.havan.com.br/dadosmestre/api/Filial/BuscarFiliaisProximas?pagina=1&itensPorPagina=100&endereco=SC');
...
export default SantaCatarinaHavanMap;
