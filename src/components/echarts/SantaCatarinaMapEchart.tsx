...
import exportSvgImage, {santaCatarinaSvgString} from '../svg/utils';
...
const mapOption = {
  ...
  series: 
    {
      id: 'sc-population',
      name: 'Poulação Santa Catarina 2024',
      type: 'map',
      map: 'SantaCatarina_map',
      roam: true,
      itemStyle: {
        emphasis: {
           areaColor: null,
...
const AnimatedSantaCatarinaMapEchart = () => {
  const chartRef = useRef(null);
  const echartRef = useRef<null |  echarts.ECharts>(null);
  const [isMap, setIsMap] = useState(true)

  useEffect(() => {
    echarts.registerMap('SantaCatarina_map', { svg: exportSvgImage(santaCatarinaSvgString), } );
...
export default AnimatedSantaCatarinaMapEchart;
